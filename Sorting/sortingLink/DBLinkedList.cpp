...
class DBLinkedList {
	...
	public:
		...
		void sortInsert() {
			int row = 1;
			node *temp,*itemp,*jtemp;
			cout << count ;
			for(itemp = head; itemp!=NULL; itemp=itemp->link) {
				temp->data = itemp->data;
				//cout << "1" ;
				for(jtemp=itemp->plink; jtemp!=NULL; jtemp=jtemp->plink) {
					//cout << "2" ;
					if(jtemp->data > temp->data) {
						jtemp->link->data = jtemp->data;
						jtemp->data = temp->data;
					}
				}
			}
		}
