...
class DBLinkedList{
	...
	public:
		...
		void binarySearch(int value){
			...
			int num_first = 1;
			int num_last = count;
			int num_mid = (num_first+num_last)/2;
			int row_mid = 1;
			int row = 1;
			int check;
			...
			while(num_first <= num_last){
				cout << row << "\t|\t" << num_first << "\t|\t" << num_last << "\t|\t" << num_mid << endl;
				if(value > mid->data){
					num_first = num_mid+1;
					check = 1;
					...
					cout << "Found " << value << "  || Index " << num_mid << endl;
					row = -1;
					break;
				}
				row_mid = num_mid;
				...
				if(check == 1){
					for(mid=mid;mid!=NULL;mid=mid->link){
						if(row_mid == num_mid){
							break;
						}
						row_mid++;
					...
					for(mid=mid;mid!=NULL;mid=mid->plink){
						if(row_mid == num_mid){
							break;
						}
						row_mid--;
